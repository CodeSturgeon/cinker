...
var cfgLauncher = function(cfg_path){
  ...
  cli.debug(util.inspect(cfg));

  cfg.host = cfg.host || 'localhost';
  cfg.port = cfg.port || 5984;

  if (cfg.db_name === undefined || cfg.profile === undefined){
    cli.error('bad config file... ignoring '+cfg_path);
    return;
  }
  ...
  req.on('response', function(resp){
    if (resp.statusCode != 200){
      ...
      cli.error(cfg.profiles_uri+' ('+cfg_path+')');
      cli.error('exiting');
      return;
    }
    var ret = '';
...
exports.cfgLauncher = cfgLauncher;
